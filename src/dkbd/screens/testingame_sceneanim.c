...
void testIngame_updateSubFloorAnimation();

void testIngame_updateSceneAnimation() {
	if (repeats_delayed(TestIngame_DATA->screenData.frameCount, 0, 0, repeats_1_each_2_mode)) {
		++TestIngame_DATA->screenData.sceneAnimationFrame;
		if (TestIngame_DATA->screenData.sceneAnimationFrame > 3) {
			TestIngame_DATA->screenData.sceneAnimationFrame = 0;
		}
		testIngame_updateSubFloorAnimation();
	}
}

void testIngame_updateSubFloorAnimation() {
	switch (TestIngame_DATA->screenData.sceneAnimationFrame) {
		case 3:
